...
//TODO: put MellowSignalHandler in DataModel Constructor... or:
// build by default.
...
public class PlayerSignalHandler {
	...
	public void updateDataModelSignalsWithPlayedCard(String playerName, String card) {
		...
		if(dataModel.getBid(playerIndex) == 0) {
			...
			handleSignalsFromActiveMellow(playerIndex, card);
		} else /*if no one said mellow */ {
			
			
			
			if(playerIndex != Constants.CURRENT_AGENT_INDEX) {
				
				//MellowVoidSignals.updateDataModelSignalsWithPlayedCard(playerName, card);
			}
		}
	...
	public void handleSignalsFromActiveMellow(int playerIndex, String card) {
		...
		int cardNum = dataModel.getMellowCardIndex(card);
		...
		if(throwNumber == 0) {
			...
			if(CardStringFunctions.getIndexOfSuit(card) == suitLeadIndex) {
				...
				if(dataModel.getCurrentFightWinningCard().equals(card)) {
					...
					int maxRankIndex = dataModel.getRankIndex(dataModel.getCurrentFightWinningCard());
				...
				if(mellowSuitPlayed == Constants.SPADE && throwNumber >= 1) {
					...
					if(dataModel.getCurrentFightWinningCard().equals(card)) {
						...
						int rankCurrentFightWinner = dataModel.getRankIndex(dataModel.getCurrentFightWinningCard());
...
public boolean mellowSignalledNoCardOverCardSameSuit(String inputCard, int mellowPlayerIndex) {
	...
	public boolean mellowPlayerMayBeInDangerInSuit(int mellowPlayerIndex, int suitIndex) {
		...
		String currentlyWinningCard = dataModel.getCurrentFightWinningCard();
