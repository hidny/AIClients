...
public class EuchreAIListener implements GamePlayerInterface {//change to final
	public static final String END_OF_TRANSMISSION = "**end of transmission**";

	public static final String START_MSG = "Starting Euchre!";
	...
	public static final int NUM_CARDS_AT_START = 5;
	...
	private EuchreAIDeciderInterface gameAIAgent = null;
	
	private String currentPlayerName = null;
	
	public EuchreAIListener(long aiLevel, boolean isFast) {
		gameAIAgent = EuchreAIDeciderFactory.getAI(aiLevel, isFast);
	}
	...
	public String getClientResponse(String serverMessage) {
		...
		if( serverMessage.length() > 0) {
			if(serverMessage.startsWith(PUBLIC_MSG)) {
				...
				if(serverMessage.contains(YOUR_CALL)) {
					...
					//"From Game(private): You declared D trump alone."
					
				} else if(cardsTemp.length > 0 && isACard(cardsTemp[0])) {
					if(cardsTemp.length == NUM_CARDS_AT_START) {
